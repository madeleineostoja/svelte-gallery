...
export default function ImageOverlay({
  ...
  const classNames = [styles['image-overlay']];
  isSelected && classNames.push(styles['is-selected']);
  selectMode && classNames.push(styles['select-mode']);

  return (
    <div className={classNames.join(' ')} onClick={handleClick} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <Checkbox
        className={styles['image-checkbox']}
        highlight={selectMode}
        hover={isHovering}
        onChange={handleChange}
        checked={isSelected}
      />
      <button type="button" onClick={handleMagnifyClick} className={styles['image-magnify']} >
